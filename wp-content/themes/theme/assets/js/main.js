...
jQuery(document).ready(function ($) {
    ...
    /* PRODUCT-PAGE */
    //PRODUCT SWIPER
    const swiperCard = new Swiper('.single-product .product-main-swiper ', {
        ...
        thumbs: {
            swiper: {
                el: '.single-product .product-thumbs-swiper .swiper',
                slidesPerView: 4,
                direction: 'horizontal',
                spaceBetween: 20,
                breakpoints: {
    ...
    // TABS
    const productBottomBlock = $('.single-product .single-product__bottom-block');
    const productBottomBlockTabs = productBottomBlock.find('.tab');
    const productBottomBlockContent = productBottomBlock.find('.tab-block');
    productBottomBlockTabs.each(function () {
        $(this).click(function (){
            let dataTab = $(this).data('tab');
            $(this).addClass('active').siblings().removeClass('active');

            productBottomBlockContent.each(function () {
                let contentDataTab = $(this).data('tab');
                if(contentDataTab === dataTab){
